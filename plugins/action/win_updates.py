            ...
            if current_updates and not new_updates:
                ...
                for update_id in current_updates:
                    ...
                    if not update_info:
                        ...
                        display.vv(
                            f"Update {update_id} not found in update info, flagging as potential loop.",
                            host=task_vars.get('inventory_hostname', None)
                        )
                        updates_to_flag.append(update_id)
                        continue
                    
                    # First, check if THIS specific UpdateID was successfully installed in a previous round.
                    # This is the most direct check and handles the KB5007651 case on Windows Server 2025.
                    ...
                    # If same UpdateID wasn't installed, check for newer versions of the same KB number.
                    # This handles cases where Microsoft re-uses KB numbers with different UpdateIDs.
                    kb_numbers = update_info.get('kb', [])
                    should_mark_as_successful = False
                    
                    if kb_numbers:
                        # Check current round's search results for newer version of same KB
                        for other_update_id, other_update_info in update_result.updates.items():
                            if other_update_id != update_id:
                                ...
                                if set(kb_numbers) & set(other_kb_numbers):
                                    should_mark_as_successful = True
                                    display.vv(
                        ...
                        if not should_mark_as_successful:
                    ...
                    if not should_mark_as_successful:
                        # This UpdateID was not successfully installed and no newer version found, flag as loop
                        updates_to_flag.append(update_id)
